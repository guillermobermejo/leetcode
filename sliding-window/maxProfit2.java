...
Algorithm:
    ...
    declare:
        1 int value     name: max                                   value: 0
        1 int[] array   name: max_from      size: prices.length     value: defaults to 0
...
Calculating max:
    ...
    max=0       iteration from: i=0 - i=prices.length-2 (i=4)

                                        sell price        buy price
    when i = 0 -> max = Math.max(max, (max_from[1] (6) - prices[0] (7)) (-1)        max=0
    when i = 1 -> max = Math.max(max, (max_from[2] (6) - prices[1] (1)) ( 5)        max=5
    when i = 2 -> max = Math.max(max, (max_from[3] (6) - prices[2] (5)) ( 1)        max=5
    when i = 3 -> max = Math.max(max, (max_from[4] (6) - prices[3] (3)) ( 3)        max=5
    when i = 4 -> max = Math.max(max, (max_from[5] (4) - prices[4] (6)) (-2)        max=5
*/
