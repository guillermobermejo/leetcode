...
Visualization of sections:                                 c:
    board:                                                  |
        ...
        where:                                                                  add: maps[0][i] maps[1][j] maps[2][quadrants[i][j]]
        maps[?][][]:            maps[][?][]:                    maps:[][][?]
...
Approach:
    ...
    if ascii calc is >= 0 (1-9) increment the value of the approriate row column and quadrant to register this number
        maps[0][i][c]++
        maps[1][j][c]++
        maps[2][quadrants[i][j][c]++
    if at any point it is NOT 0 then it is present and NOT valid
    
Algorithm:
    declare & initialize:
        1 int[][] array     name: quadrants     value: HARDCODE     notes: O(1) access at any i,j position to a quadrant
            ...
            check & increment:
                ...
                IF maps[1][j][c]++ != 0  // maps[1][0][4] if the value at index 4 of the first column in columns != 0
                    return false
                                ...
                                0 ------> 1
                IF maps[2][quadrants[i][j]][c]++ != 0   // maps[2][0][4] if the value at index 4 of the first quadrant 
        ...
        for (int i = 0; i < board.length; i++) {
            for(int j = 0; j < board.length; j++) {
                ...
                if (c >= 0) {
                    ...
                    if (maps[2][quadrants[i][j]][c]++ != 0) return false;
                }
            }
        }
        return true;
