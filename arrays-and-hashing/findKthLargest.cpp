...
Runtime: 54ms (beats 99.01%)
Memory: 57.6mb (beats 93.68%)
...
Approach:
	...
	return -1		// this line is only reached in the case of a mismatch where k > nums[].size()
...
Algorithm:
	...
	for loop: (int i = (sizeof(pos_map) / sizeof(int))-1; i > -1; i--)	// traverse right to left (largest to smallest for +)
		if check:
	...
	for loop: (int i = 1; i < (sizeof(pos_map) / sizeof(int)); i++)		// traverse left to right (largest to smallest for -)
		if check:
...
public:
    int findKthLargest(vector<int>& nums, int k) {
        int pos_map[10000+1] = {0};             // int[] manually 
        int neg_map[10000+1] = {0};
        ...
        for (int i = (sizeof(pos_map) / sizeof(int))-1; i > -1; i--) {
            if (pos_map[i] != 0) {
                while (pos_map[i]-- != 0)
                    if (k-- == 1) return i;
			}
