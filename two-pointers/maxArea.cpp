...
Runtime: 51ms (beats 93.41%)
Memory: 61.4mb (beats 65.44%)
...
Approach:
    ...
    pointers will start off max range i=0 to i=height.size()-1
...
Algorithm:
    declare:
        ...
        1 int value     name: r             value: height.size()-1      notes: right pointer
...
public:
    int maxArea(vector<int>& height) {
        ...
        while(l < r) {
            int range = r - l;
            int area = min(height[l], height[r]) * range;
